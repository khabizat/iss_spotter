...
const fetchCoordsByIP = function(ip, callback) {
  ...
  request(`https://api.ipbase.com/json/?apikey=75fMf3SMmUsZMooqozkf0xoFHtpQ3UL5wUZfx032&${ip}`, (error, response, body) => {
    // inside the request callback ...
    // error can be set if invalid domain, user is offline, etc.
    if (error) {
      callback(error, null);
...
const fetchISSFlyOverTimes = function(coords, callback) {
  // get latitude and longitude from coords object
  const lat = coords.latitude;
  const lon = coords.longitude;
  // use request to fetch ISS data from JSON API
...
const nextISSTimesForMyLocation = function(callback) {
  fetchMyIP((error, ip) => {
    ...
    fetchCoordsByIP(ip, (error, location) => {
      ...
      fetchISSFlyOverTimes(location, (error, issData) => {
        if (error) {
          console.log('It didn\'t work', error);
          return;
        }
